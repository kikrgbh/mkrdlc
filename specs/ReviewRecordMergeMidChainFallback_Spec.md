...
**General rule, closing a defect class three straight G1 re-check rounds each found a fresh instance
of (§13, rev 3 on `D`, rev 4 on `base`, rev 5 re-check on `M2`/`M`):** per `rrf_commit()`'s own
actual behavior (a plain commit helper, `tests/hooks_test.sh` ~line 1922), **no commit in any
fixture below carries its own exact-match review record unless that fixture's own prose explicitly
says a record is planted for it** — this matches how every pre-existing `TC-RRF-01..20` fixture
already works, and is the actual ground truth `fixture_repo`-based tests build from, not a new
convention invented here. This applies uniformly to *every* commit named in every fixture below,
including the top-level `sha` `find_review_record` is called on directly — §6's stated check
ordering runs the exact-match check first, for every `sha`, with no exception for the outermost
call. Stating this once here, rather than repeating a per-commit "has no own record" caveat that
kept being present for some commits and silently missing for others across five revisions, is the
actual fix: the previous five rounds' real defect was never any single missing caveat, it was
relying on caveats at all instead of a blanket default that can't be selectively forgotten.
...
| 5 | mkr-spec-reviewer (G1, re-check of rev 5) | NOT READY (2 blocking) | Re-verified TC-RRF-22 sound (unchanged from rev 4, no new gap). Found the *same defect class* recurring in two more places rev 5's own fix didn't touch: (1) `TC-RRF-26`'s top-level `sha`, `M2` — the fixture never states `M2` itself lacks an exact-match record, even though §6's ordering runs the exact-match check first for every `sha` including the outermost call; if `M2` resolves there, the ancestor-check/AD-2/AD-3/`F`/`base` chain the test exists to prove is never reached. Rev 5's own §13 entry claiming this fixture's precondition chain was closed "completely" was therefore inaccurate. (2) `TC-RRF-21`'s `M` — the fixture states `M`'s *second parent* carries a record but never states `M` itself doesn't, the identical gap, present since rev 1 and never caught across three prior rounds that each explicitly re-traced this exact test. Fixed in rev 6, systemically rather than per-commit: added a general rule at the top of §9 stating that, matching `rrf_commit()`'s own real behavior, no commit in any fixture carries a record unless the fixture text explicitly plants one — closing this defect class for every commit in every fixture (`M2` and `M` included) in one place, instead of continuing to require (and evidently keep missing) a per-commit caveat. |
| 6 | mkr-spec-reviewer (G1, re-check of rev 6) | READY | Confirmed the new general rule (§9) unambiguously covers the outermost `find_review_record` call target, not just recursed commits — it names that case explicitly, closing the exact ambiguity that caused the rev-3/4/5 recurring miss. Independently re-hand-traced TC-RRF-21, TC-RRF-22, and TC-RRF-26 against the real, current `.claude/hooks/lib/reviewrecord.sh` and `tests/hooks_test.sh`'s `rrf_commit()` helper (not just the spec's own prose) and confirmed the general rule's factual premise holds against every existing `TC-RRF-01..20`/`TC-MRF-01..02` fixture, and that all three new fixtures genuinely resolve as claimed, relying on the rule rather than redundant per-commit callouts for `M`/`M2` specifically. Confirmed the surviving explicit callouts on `D`/`base` are consistent restatements, not contradictions. No blocking findings. Non-blocking: the general rule's own motivating clause said "four straight rounds" when only three rounds (3, 4, 5) were involved (round 5 found two instances in one round, not two rounds) — corrected in this same revision. |
