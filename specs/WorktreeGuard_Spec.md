...
| | |
|---|---|
| **Status** | DRAFT rev 5 (rev 4 was `ACCEPTED`, then found `NOT READY` at a G4 re-review — see §13; §9-only content fix, resubmitted for G1 only, §6/§7/§8 untouched so no G3 re-review needed) |
| **Depth** | Deep |
| **Author** | agent |
| **Approver** | kikrgbh |
...
| `TC-WGSPEC-01` | `specs/WorktreeGuard_Spec.md` exists, `Status` field reads `ACCEPTED` | Done — `ACCEPTED rev 4 (kikrgbh, 2026-08-11)` |
...
- [ ] `specs/WorktreeGuard_Spec.md` reaches `Status: ACCEPTED (kikrgbh, <date>)` via G1
      ...
      finding (a stale §9 row, self-inconsistent with this very line) required a rev-5 content fix;
      this box stays unchecked until rev 5 is re-approved. §6/§7/§8 are untouched by the rev-5 fix,
      so no further G3 re-review is needed — only G1.
...
| 5 | pending (G1 only — §6/§7/§8 untouched, no G3 re-review needed) | — | Fixes the G4 finding above: `TC-WGSPEC-01`'s row corrected to `Done — ACCEPTED rev 4 (kikrgbh, 2026-08-11)`. Also fixed both non-blocking notes: `TC-WGSPEC-02` now cites all three G3 records (`rev2`/`rev3`/`rev4`); all three "no `TC-WG-14`" occurrences (§3, §9, AC5) now also state "or bare `TC-WG-15`". Resubmitted for G1 only. |
