...
| | |
|---|---|
| **Status** | ACCEPTED rev 11 (kikrgbh, 2026-08-11 — explicit human override: accepted with 2 disclosed, deferred G1 findings not resolved in this revision chain; see §13 row 11) |
| **Depth** | Deep |
| **Author** | agent |
| **Approver** | kikrgbh |
...
| `TC-WGSPEC-06` | AC2 itself: `mkr-design-reviewer` and `mkr-architecture-reviewer`, each reading only the current `§7.3`/`§7.4` deny/warn text (no other context), independently state in their G3 verdict which of the two conditions each message describes; a recorded mismatch or "cannot tell" from either reviewer fails this case and triggers the `TC-WGSPEC-05` wording fix | Done as of rev 2's §7.3/§7.4 text — **open question, not re-certified**: rev 7-11 materially changed §7.3/§7.4 (the ambiguous-fallback correction) after this verdict was recorded; whether it still holds against the current text is undetermined, disclosed per §13 row 11's human override rather than resolved; see `.mkr/designs/WorktreeGuard-rev2.md` |
...
- [x] `specs/WorktreeGuard_Spec.md` reaches `Status: ACCEPTED (kikrgbh, <date>)` via G1
      (`mkr-spec-review`) — done, `ACCEPTED rev 11 (kikrgbh, 2026-08-11)`, via explicit human
      override after 5 review rounds (§13 rows 6-11): the core finding (AC1, both halves of the
      ambiguous-fallback regex accurately and consistently described) is independently confirmed
      correct; 2 remaining G1 findings (ADR Context staleness; the AC2 open question below) are
      disclosed and deferred rather than resolved in this chain.
- [ ] G3 design gate run (`mkr-design-reviewer` + `mkr-architecture-reviewer`, independent, parallel)
      ...
      verdict either way. — rev 4 achieved this against pre-correction §6/§7 text
      (`.mkr/designs/WorktreeGuard-rev4.md`); **explicitly deferred, not re-run, per the same human
      override** — rev 7-11's ambiguous-fallback correction changed §6/§7 materially enough that
      AC2's continued validity is a genuinely open question (see the next bullet), left open rather
      than gate-blocking this merge.
- [ ] AC2 required no message-wording fix (rev-2 design gate verdict) — **disputed, not settled**:
      this bullet and the one above currently contradict each other (this one says "unaffected by
      later content fixes"; the one above says re-review is needed). Left unreconciled and disclosed
      per the human override (§13 row 11) rather than silently resolved either way. `TC-WGSPEC-05`'s
      applicability is correspondingly also open, not confirmed N/A.
- [x] `docs/adr/0012-worktree-guard-policy-tiers.md` filed, formalizing AD-1 through AD-5, the
      documented-but-deferred advisory-tier gap, and (rev 7-10) the ambiguous-fallback correction —
      filed and updated; **known follow-up, deferred**: the ADR's own Context section is a stale
      summary of the spec's real acceptance history (omits the rev-2 G4 rejection, rev 3/4 G3
      re-reviews, the actual rev-6 acceptance/merge, and the phase-9 audit FAIL) — disclosed in §13
      row 11, not fixed in this revision chain.
...
| 11 | NOT READY (G1, 2 blocking) — **human override: accepted anyway, see below** | `mkr-spec-reviewer`; `kikrgbh` | Closed the recurring defect class structurally instead of patching a third row's snapshot: `TC-WGSPEC-02`'s status column no longer restates "G3 half done"/"audit half pending" as a snapshot — it now points to §11 DoD's own G3 and grounding-audit checkboxes as the durable source of truth, the same structural fix rev 6 already applied to `TC-WGSPEC-01`. Reviewer confirmed this fix is genuinely durable, and did an exhaustive document-wide sweep as instructed (checking `TC-WGSPEC-03`/`04`/`05`/`06`/`07`/`08` too, not just the two already-flagged rows) — found none of those stale, and confirmed row 11's own drafting had briefly misattributed `TC-WGSPEC-01`'s history to `TC-WGSPEC-02` before self-correcting. But the same sweep surfaced two NEW findings, one meta-consistency (the ADR's Context section is a stale summary of the spec's own acceptance history — §13 itself is the accurate account) and one a genuine open question (does AC2/`TC-WGSPEC-06`'s message-clarity certification, from rev 2's G3 round, still hold now that a denial can be triggered by the ambiguous-fallback match rather than a real commit/checkout/switch — DoD bullet 2 says re-review is needed while bullet 3/`TC-WGSPEC-06` independently assert it's already settled, an unresolved internal contradiction). **Human decision (`kikrgbh`), asked explicitly given 5 consecutive review rounds on what began as a single documentation-accuracy fix (§13 row 6's `AC1 NOT VERIFIED` finding): stop the G1 revision loop here.** The core finding — both halves of the ambiguous-fallback regex now accurately and consistently described everywhere in the document — is independently confirmed correct across the last three rounds; the two new findings are about the spec's own paperwork self-consistency, not the guards' actual behavior (unchanged, still 180/180 passing, still the same deliberately-conservative design it always was), and are recorded here as explicit, disclosed, deferred follow-ups rather than silently resolved or silently dropped: (a) `docs/adr/0012-worktree-guard-policy-tiers.md`'s Context section needs updating to reflect the spec's real acceptance history (G4 rejection at rev 2, re-reviews at rev 3/4, actual acceptance/merge at rev 6, the phase-9 audit FAIL); (b) whether AC2/`TC-WGSPEC-06` needs actual G3 re-certification post-ambiguous-fallback-correction is an open question, not yet answered either way — `§11` DoD bullets 2 and 3 both still stand, contradicting each other, disclosed rather than reconciled. |
