...
same computation (spec-gate.sh's own ASK reason, "no spec on this branch has reached ACCEPTED yet
— proceeding treats this as Quick-depth", citing docs/DESIGN.md §4 G1). This is a distinct outcome
from the next one below, not an instance of it — do not stop here for a branch that never drafted
a spec at all.
